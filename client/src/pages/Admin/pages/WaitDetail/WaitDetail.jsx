...
export const WaitDetail = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`${api}/api/v1/boards/${id}`, {
            headers: {
    ...
    const approve = () => {
        const api = process.env.REACT_APP_API_URL;
        axios.post(`${api}/api/v1/admin/boards/${id}/approve`, {}, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`
            },
            body: {
        ...
        <section className="container">
            ...
            <div className="wait-header detail-header">
                ...
                <div className="wait-category">
                    <div>카테고리</div>
                    <div>{data?.categories.map((category, index) => <span key={index}>{category}</span>)}</div>
                </div>
                <div className="wait-hashtag">
                    <div>해시태그</div>
                    <div>{data?.tags.map((tag, index) => <span key={index}>{tag}</span>)}</div>
                </div>
                <div className="wait-img">
                    <div>대표사진</div>
                    <div><img src={data?.thumnail} alt="대표사진"/></div>
                </div>
            </div>
            ...
            <div className="wait-body detail-header">
                ...
                <div className="wait-source">
                    <div>출처</div>
                    <div>{data?.sources.map((source, index) => <span key={index}>{source}</span>)}</div>
                </div>
            </div>
